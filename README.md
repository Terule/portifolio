...
The Docker image uses Next.js standalone output, runs as a non-root user, and includes only runtime files. Coolify can probe `GET /api/health`, which returns `{"status":"ok"}`. No environment variables are required for the current static portfolio.
